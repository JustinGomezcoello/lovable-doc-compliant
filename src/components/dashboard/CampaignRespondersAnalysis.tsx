...
export const CampaignRespondersAnalysis = ({ 
  ...
  const fetchRespondersDetails = async () => {
    ...
    try {
      ...
      console.log(`   📊 Total de cédulas a consultar: ${numericCedulas.length}`);      for (let i = 0; i < numericCedulas.length; i += CHUNK_SIZE) {
      ...
      }      console.log(`   🎯 Total respondedores encontrados (filtrados por DiasMora): ${allResponders.length}`);

      // ✅ ELIMINAR DUPLICADOS POR CELULAR (mantener solo uno por persona)
      // Si hay múltiples registros con el mismo celular y DiasMora, mantener el primero
      ...
      ).length;      // 3. PERSONAS QUE ENVIARON COMPROBANTE (validación de 2 condiciones principales)
      ...
      console.log(`   📋 Total con comprobante válido: ${sentReceipt} de ${uniqueResponders.length} (${((sentReceipt / uniqueResponders.length) * 100).toFixed(1)}%)`);// 4. PERSONAS QUE YA NO DEBEN NADA (saldo = 0 en el sistema)
      ...
      const noDebtAnymore = uniqueResponders.filter(r => {
        ...
        return false;
      }).length;      // 5. CALCULAR DEUDA REAL PENDIENTE
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                ...
                </div>                <div className="bg-indigo-50 p-3 rounded-lg text-center">
              ...
              </div>{/* Tabla de respondedores */}
              {analysis.responders.length > 0 && (
                <div className="mt-4">
                  <h4 className="font-semibold mb-2">Respondedores ({analysis.totalResponders})</h4>
                  <div className="overflow-x-auto max-h-96 overflow-y-auto">
                    <table className="w-full text-sm border-collapse">                      <thead className="sticky top-0 bg-gray-100">
